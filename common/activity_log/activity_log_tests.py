...
class ActivityLogTests(unittest.TestCase):
    ...
    def test_01_can_add_processed_lines_correctly(self):
        ...
        for msg_id in msg_ids:
            self.assertEqual(
                self._activity_log.is_msg_id_already_processed(client_id, msg_id), False
            )
        ...
        for msg_id in msg_ids:
            self.assertEqual(
                self._activity_log.is_msg_id_already_processed(client_id, msg_id), True
            )
    ...
    def test_03_can_properly_log_two_ends(self):
        ...
        self.assertEqual(
            self._activity_log._get_amount_of_ends(client_id, end_logging="0"), 0
        )
        self.assertEqual(
            self._activity_log._get_amount_of_ends(client_id, end_logging="1"), 0
        )

        self._activity_log.log_end(client_id, "20", end_logging="0")
        self.assertEqual(
            self._activity_log._get_amount_of_ends(client_id, end_logging="0"), 1
        )
        self.assertEqual(
            self._activity_log._get_amount_of_ends(client_id, end_logging="1"), 0
        )
        ...
        self.assertEqual(
            self._activity_log._get_amount_of_ends(client_id, end_logging="1"), 1
        )
    ...
    def test_04_can_properly_log_two_ends_for_multiclient(self):
        ...
        self.assertEqual(
            self._activity_log._get_amount_of_ends(client_id_2, end_logging="0"), 0
        )
        self.assertEqual(
            self._activity_log._get_amount_of_ends(client_id_2, end_logging="1"), 0
        )

        self._activity_log.log_end(client_id_2, "22", end_logging=0)
        self.assertEqual(
            self._activity_log._get_amount_of_ends(client_id_2, end_logging="0"), 1
        )
        self.assertEqual(
            self._activity_log._get_amount_of_ends(client_id_2, end_logging="1"), 0
        )
        ...
        self.assertEqual(
            self._activity_log._get_amount_of_ends(client_id_2, end_logging="1"), 1
        )
    ...
    def test_06_ends_admit_duplicates_if_end_logging_is_different(self):
        ...
        end_was_repeated = self._activity_log.log_end(
            client_id, msg_id, end_logging="0"
        )
        ...
        end_was_repeated = self._activity_log.log_end(
            client_id, msg_id, end_logging="1"
        )
    ...
    def test_01_can_recover_ends_satate_correctly(self):
        ...
        expected = {client_id: 2, client_id_2: 1}
    ...
    def test_02_can_recover_middleware_correctly(self):
        ...
        expected = {queue_name_1: (msg_1, 1), queue_name_2: (msg_2 + msg_3, 2)}
    ...
    def test_03_can_recover_for_multi_end_correctly(self):
        ...
        expected_1 = {client_id: 1, client_id_2: 4}

        expected_2 = {client_id: 2, client_id_2: 3}

        self._activity_log.log_end(client_id, "0", end_logging="0")
        [
            self._activity_log.log_end(client_id_2, f"{i}", end_logging="0")
            for i in range(1, 5)
        ]

        [
            self._activity_log.log_end(client_id, f"{i}", end_logging="1")
            for i in range(2)
        ]
        [
            self._activity_log.log_end(client_id_2, f"{i}", end_logging="1")
            for i in range(2, 5)
        ]
    ...
    def test_01_con_recover_handler_state_correctly_for_empty_finished_queries(self):
        ...
        expected = {
            client_id: [
                queue_name,
                connection_id,
                int(last_ack_message),
                finished_queries,
            ]
        }

        self._activity_log.log_for_client_handler(
            client_id, queue_name, connection_id, last_ack_message, finished_queries
        )
