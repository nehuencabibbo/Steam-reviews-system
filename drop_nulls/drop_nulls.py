...
class DropNulls(): 
    ...
    def __handle_games_end_transmission(self):
        ...
        for platform in PLATFORMS:
            ...
            self._middleware.publish(END_TRANSMISSION_MESSAGE, f"{node_id}_{self._config['Q1_PLATFORM']}")

        # Q2, Q3, Q4, Q5
        for i in range(2, 6):
            self._middleware.publish(END_TRANSMISSION_MESSAGE, self._config[f"Q{i}_GAMES"])
    ...
    def __handle_games(self, delivery_tag: int, body: bytes):
        ...
        for platform, platform_index in PLATFORMS.items(): 
            ...
            if platform_supported.lower() == 'true':
                ...
                self._middleware.publish(message, f"{node_id}_{self._config['Q1_PLATFORM']}")
        ...
        self._middleware.publish(message, self._config["Q2_GAMES"])
        ...
        for i in range(3, 6):
            self._middleware.publish(message, self._config[f"Q{i}_GAMES"])
    ...
    def __handle_reviews(self, delivery_tag: int, body: bytes):
        ...
        if body == END_TRANSMISSION_MESSAGE:
            ...
            for i in range(3, 6):
                self._middleware.publish(END_TRANSMISSION_MESSAGE, self._config[f"Q{i}_REVIEWS"])
        ...
        for i in ['3', '5']: 
            ...
            self._middleware.publish(message, self._config[f"Q{i}_REVIEWS"])
        ...
        self._middleware.publish(message, self._config[f"Q4_REVIEWS"])
