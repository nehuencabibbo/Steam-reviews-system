...
class Watchdog:
    def __init__(self, middleware: TCPMiddleware, config: dict):
        ...
        self._leader_election = LeaderElection(
            self._node_id, self._election_port, self._amount_of_monitors
        )
        self._leader_discovery = LeaderDiscoveryService(
    ...
    def _watchdog_setup(self):
        ...
        with open("watchdog_status/monitor_names.json", "r") as file:
            ...
            self._peers = {
                ...
                if key != f"{WATCHDOG_NAME_PREFIX}{self._node_id}"
            }
    ...
    def _monitor_nodes(self):
        peer_listener_thread = threading.Thread(
            target=self._listen_for_peers, daemon=True
        )
    ...
    def _listen_to_leader(self):
        ...
        leader_addr = (
            f"{WATCHDOG_NAME_PREFIX}{leader_id}",
            self._leader_comunicaton_port,
        )
